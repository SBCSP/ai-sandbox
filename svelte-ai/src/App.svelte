<script>
  ...
  async function loadChat(chatId) {
    try {
      const res = await fetch(`http://localhost:5001/history?chat_id=${chatId}`);
      const data = await res.json();
      if (data.messages) {
        chatHistory = data.messages.map(msg => ({
          role: msg.role,
          content: msg.content,
          image: msg.role === 'user' && msg.content.startsWith('Uploaded image:') ? null : undefined // Handle images later
        }));
  ...
  async function clearChat() {
    if (!chatId) return;
    try {
      const res = await fetch(`http://localhost:5001/history/${chatId}`, { method: 'DELETE' });
      const data = await res.json();
      if (res.ok) {
        chatHistory = [];
...
<main>
  ...
  <div class="chat-controls">
    <select on:change={(e) => loadChat(e.target.value)} disabled={loading}>
      <option value="">Select a past chat</option>
      {#each availableChats as chat}
        <option value={chat.chat_id}>
          {chat.last_message} ({new Date(chat.timestamp).toLocaleString()})
        </option>
      {/each}
    </select>
    <button on:click={clearChat} disabled={loading || !chatId}>Clear Chat</button>
    <button on:click={startNewChat} disabled={loading}>New Chat</button>
  </div>
    ...
    {#each chatHistory as { role, content, image }, i}
      <div class="message {role}">
        ...
        {:else}
          ...
          {#if image}
            <img src={image} alt="Uploaded image" class="preview" />
          {/if}
        {/if}
      </div>
    {/each}
