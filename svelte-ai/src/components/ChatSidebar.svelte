<script>
    ...
    async function deleteChat(chatId) {
      ...
      if (!confirm(`Are you sure you want to delete the chat "${availableChats.find(chat => chat.chat_id === chatId)?.last_message}"?`)) {
  ...
  <div class="sidebar">
        ...
        {#each availableChats as chat}
          ...
          >
            <span class="chat-text">
              {chat.last_message.slice(0, 10) + (chat.last_message.length > 10 ? '...' : '')} 
    ...
    <button on:click={() => onSelectChat('')} disabled={loading}>Clear Selection</button>
  </div>
